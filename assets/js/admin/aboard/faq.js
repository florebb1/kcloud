...
function notice(pageNum, row) {
    var html = "";
    $.ajax({
        url: '/adminapi/fetchFaq?row='+row+'&page='+pageNum,
        type: 'GET',
        dataType: 'JSON',
            ...
            if(response.list.length > 0) {
                for (var i = 0; i < response.list.length; i++) {
                    html += '<tr>'
                        ...
                        + '<a href="/admin/aboard/faq_write?wr_id='+response.list[i].wr_id+'" class="btn btn-info btn-sm">수정</a>'
                        + '</td>'
                        + '</tr>';
                }
                $("#content").empty();
                ...
                $('#show_paginator').bootpag({
                    ...
                    location.href='/admin/aboard/faq?pageNum='+num+'&row='+row;
                });
            }else {
                html ='<tr><th colspan="11" scope="row"><div class="bbs-none d-flex justify-content-center align-items-center">등록된 FAQ가 존재하지않습니다.</div></th></tr>';
                $("#content").empty();
                $("#content").append(html);
            }
        }
...
function notice_del(id) {
    if(confirm("해당 공지를 삭제하시겠습니까?")) {
        $.ajax({
            url: '/adminapi/deleteFaq?wr_id='+ id,
            type: 'GET',
            dataType: 'JSON',
            success: function (response) {
