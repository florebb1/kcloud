...
function notice(pageNum) {
    ...
    $.ajax({
        url: '/api/fetchQuality?pageNum='+pageNum+'&searchType='+searchType+'&searchWord='+searchWord+'&type=',
        type: 'GET',
        dataType: 'JSON',
            ...
            if(response.totalCount > 0) {
                ...
                for (var i = 0; i < response.list.length; i++) {
                    ...
                    html += '<tr onclick="' + ahref + '">'
                        ...
                        + '</div>'
                        + '<div class="col-12 border-top py-2"><div class="ellipsis-multis">' + response.list[i].wr_content + '</div></div>'
                        + '</div>'
                        + '</div>'
                        + '</div>'
                        + '</td>'
                ...
                $('#show_paginator').bootpag({
                    ...
                    location.href='/page/etc?pageNum='+num;
                });
            }else {
