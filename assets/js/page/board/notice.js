...
function notice(pageNum) {
   ...
   $.ajax({
      url: '/api/fetchNotice?pageNum='+pageNum+'&searchType='+searchType+'&searchWord='+searchWord,
      type: 'GET',
      dataType: 'JSON',
      success: function (response) {
         if(response.totalCount > 0) {
            ...
            for (var i = 0; i < response.list.length; i++) {
               var ahref = "location.href='/board/notice_view?wr_id="+response.list[i].wr_id+"'";
            ...
            $('#show_paginator').bootpag({
               ...
               location.href='/board/notice?pageNum='+num;
            });
         }else {
