...
function notice(pageNum) {
    ...
    var id = $("#userSeq").val();
    $.ajax({
        url: '/api/fetchQuality?pageNum='+pageNum+'&searchType='+searchType+'&searchWord='+searchWord+'&type='+type+'&userSeq='+id,
        type: 'GET',
        dataType: 'JSON',
            ...
            if(response.totalCount > 0) {
                ...
                for (var i = 0; i < response.list.length; i++) {
                    ...
                    html += '<tr onclick="' + ahref + '">'
                        ...
                        + '</div>'
                        + '<div class="col-12 border-top py-2"><p>' + response.list[i].wr_content.replace(/(\r\n|\n|\r)/gm, "<br />") + '</p></div>'
                        + '</div>'
                        + '</div>'
                        + '</div>'
                        + '</td>'
                ...
                $('#show_paginator').bootpag({
                    ...
                    location.href='/page/iaas?pageNum='+num;
                });
            }else {
